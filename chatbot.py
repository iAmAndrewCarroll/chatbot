...
def predict_class(sentence):
    ...
    if not return_list:
        ...
        predicted_class = lemmatizer.lemmatize(return_list[0]['intent'].lower())
...
def get_response(intents_list, intents_json):
    ...
    result = None # default
    user_input = None # default

    # check if tag matches intent.json
    for i in list_of_intents:
        if i['tag'] == tag:
            result = random.choice(i['responses'])
            break
    
    # no matching intent.json, or result is None
    if result is None:
        if 'user_input' in intents_list[0]:
            user_input = intents_list[0]['user_input']
        
        # GPT3 API to respond to user input
        if user_input:
...
def generate_gpt3_response(user_input):
    ...
    prompt = f"You: {user_input}\nBot: Respond only to the user's query as an expert in diversity, equity, and inclusion"
    # print("Prompt: ", prompt)
